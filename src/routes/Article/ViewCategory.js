...
import { CATEGORY_POST_QUERY } from '../../graphql/post';
...
class ViewCategory extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      color: this.generateColors(nextProps.data.postByCategory.length)
    });
  }
  ...
  render() {
    const { data: { loading, postByCategory: posts } } = this.props;
    ...
    return (
      <div>
        <PageHeader
          title={`Kategori: ${this.props.match.params.category}`}
...
export default graphql(CATEGORY_POST_QUERY, {
  options: (props) => ({
